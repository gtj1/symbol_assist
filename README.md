...
方法一：下载后点击或在命令行中直接运行 `symbol_assist.exe` 即可.
方法二：下载 `symbol_assist.ahk`, 使用 `Auto HotKey` 编译后运行编译后生成的 `.exe` 文件. 如果需要帮助请参考 [autohotkey.com](https://www.autohotkey.com/). 你可以自行在 `symbol_assist.ahk` 中修改符号映射, 以适应你的需求.
